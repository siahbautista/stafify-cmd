...
@section('description', 'Manage employee development and talent retention.')
                        ...
                        <div class="toolkit-card square-card bg-white rounded-lg shadow-sm hover:shadow-md transition-all duration-200 border border-gray-100">
                            ...
                            <div class="toolkit-content" 
                                ...
                                @if($toolkit->icon && file_exists(public_path('HRIS/hr-toolkit/assets/crm_hr/' . basename($toolkit->icon))))
                                    <img src="{{ asset('HRIS/hr-toolkit/assets/crm_hr/' . basename($toolkit->icon)) }}" 
                                         alt="" 
    ...
    <div id="addToolkitModal" class="hidden fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
        <div class="relative min-h-screen px-4 text-center">
            ...
            <div class="inline-block w-full max-w-md p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-2xl relative">
                ...
                <form method="POST" action="{{ route('talent-toolkit.store') }}" class="mt-4">
                    @csrf
                    ...
                    <div class="space-y-4">
                                ...
                                @foreach($icons as $icon)
                                    @if($icon !== '.' && $icon !== '..')
                                        <div class="icon-option" 
                                             ...
                                             data-value="{{ $icon }}">
                                            <img src="{{ asset('HRIS/hr-toolkit/assets/crm_hr/' . $icon) }}" 
                        ...
                        <div>
                            ...
                            <select name="type" id="toolkit_type_select" class="mt-1 block w-full p-2 rounded-md border border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 transition duration-200" required onchange="updateToolkitTypeFields()">
                                ...
                                <option value="Multiple Forms + Sheets">Multiple Forms + Sheets</option>
                            </select>
                        </div>
                        <div id="toolkit_type_fields">
                            ...
                            <!-- Multiple Forms + Sheets fields -->
                            <div id="multiple_forms_sheets_group" style="display:none;">
                                <div class="mb-4">
                                    <label class="block text-sm font-medium text-gray-700 mb-2">Form URLs (up to 6)</label>
                                    <div class="space-y-2">
                                        @for($i = 1; $i <= 6; $i++)
                                            <div>
                                                <label class="block text-xs text-gray-600 mb-1">Form {{ $i }} URL</label>
                                                <input type="url" name="form_url_{{$i}}" id="form_url_{{$i}}" 
                                                       class="mt-1 block w-full p-2 rounded-md border border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 transition duration-200" 
                                                       placeholder="Enter Form {{ $i }} URL">
                                            </div>
                                        @endfor
                                    </div>
                                </div>
                                <div class="mb-4">
                                    <label class="block text-sm font-medium text-gray-700 mb-2">Sheet URLs (up to 6)</label>
                                    <div class="space-y-2">
                                        @for($i = 1; $i <= 6; $i++)
                                            <div>
                                                <label class="block text-xs text-gray-600 mb-1">Sheet {{ $i }} URL</label>
                                                <input type="url" name="sheet_url_{{$i}}" id="sheet_url_{{$i}}" 
                                                       class="mt-1 block w-full p-2 rounded-md border border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 transition duration-200" 
                                                       placeholder="Enter Sheet {{ $i }} URL">
                                            </div>
                                        @endfor
                                    </div>
                                </div>
...
function loadToolkit(responseUrl, formUrl, isApproved, cardElement, toolkitTitle = '', type) {
    ...
    // Handle Multiple Forms + Sheets type
    if (type === 'Multiple Forms + Sheets') {
        // Parse JSON arrays from form_url and response_url
        let formUrls = [];
        let sheetUrls = [];
        
        try {
            if (formUrl && formUrl !== '#') {
                formUrls = JSON.parse(formUrl);
            }
            if (responseUrl && responseUrl !== '#') {
                sheetUrls = JSON.parse(responseUrl);
            }
        } catch (e) {
            console.error('Error parsing JSON:', e);
        }
        
        // Create tabbed interface for multiple forms and sheets
        const container = iframe.parentNode;
        
        // Remove existing multiple forms/sheets UI if present
        const existingUI = document.getElementById('multipleFormsSheetsUI');
        if (existingUI) {
            existingUI.remove();
        }
        ...
        uiContainer.id = 'multipleFormsSheetsUI';
        uiContainer.className = 'w-full';
        
        // Create Form tabs container
        const formTabsContainer = document.createElement('div');
        formTabsContainer.className = 'flex flex-wrap gap-2 mb-2';
        
        // Create Sheet tabs container
        const sheetTabsContainer = document.createElement('div');
        sheetTabsContainer.className = 'flex flex-wrap gap-2 mb-4 border-b border-gray-200 pb-2';
        
        // Add Form tabs
        formUrls.forEach((url, index) => {
            if (url) {
                const tab = document.createElement('button');
                tab.className = 'px-4 py-2 rounded-t-lg bg-gray-100 hover:bg-gray-200 text-sm font-medium text-gray-700';
                tab.textContent = `Form ${index + 1}`;
                tab.dataset.type = 'form';
                tab.dataset.index = index;
                tab.dataset.url = url;
                tab.onclick = function() {
                    // Update active tab (check all tabs in both containers)
                    const allTabs = uiContainer.querySelectorAll('button');
                    allTabs.forEach(btn => {
                    ...
                    // Load form in iframe
                    iframe.classList.remove('hidden');
                    iframe.src = getEmbeddableUrl(url, 'Form');
                };
                formTabsContainer.appendChild(tab);
            }
        });
        ...
        sheetUrls.forEach((url, index) => {
            if (url) {
                ...
                tab.onclick = function() {
                    ...
                    iframe.classList.remove('hidden');
                    iframe.src = getEmbeddableUrl(url, 'Sheet');
                };
                sheetTabsContainer.appendChild(tab);
            }
        });
        
        uiContainer.appendChild(formTabsContainer);
        uiContainer.appendChild(sheetTabsContainer);
        container.insertBefore(uiContainer, iframe);
        
        // Load first form by default if available
        if (formUrls.length > 0 && formUrls[0]) {
            iframe.classList.remove('hidden');
            iframe.src = getEmbeddableUrl(formUrls[0], 'Form');
            // Activate first form tab
            const firstTab = formTabsContainer.querySelector('[data-type="form"][data-index="0"]');
            if (firstTab) {
                firstTab.classList.remove('bg-gray-100', 'text-gray-700');
                firstTab.classList.add('bg-blue-500', 'text-white');
            }
        } else if (sheetUrls.length > 0 && sheetUrls[0]) {
            iframe.classList.remove('hidden');
            iframe.src = getEmbeddableUrl(sheetUrls[0], 'Sheet');
            // Activate first sheet tab
            const firstTab = sheetTabsContainer.querySelector('[data-type="sheet"][data-index="0"]');
            if (firstTab) {
                firstTab.classList.remove('bg-gray-100', 'text-gray-700');
                firstTab.classList.add('bg-blue-500', 'text-white');
            }
        ...
        // Hide Open Form/Open Response buttons for Multiple Forms + Sheets
        openFormBtn.style.display = 'none';
        openResponseBtn.style.display = 'none';
        
        // Remove folder toggle if present
        const folderToggle = document.getElementById('folderViewToggle');
        if (folderToggle) folderToggle.remove();
        
        return; // Exit early for Multiple Forms + Sheets
    }
    ...
    // Remove multiple forms/sheets UI if present (for other types)
    const existingUI = document.getElementById('multipleFormsSheetsUI');
    if (existingUI) {
        existingUI.remove();
    }
    ...
    } else if (type === 'Multiple Forms + Sheets') {
        openFormBtn.style.display = 'none';
        openResponseBtn.style.display = 'none';
        // Remove List/Grid toggle if present
        const folderToggle = document.getElementById('folderViewToggle');
...
function openAddToolkitModal(toolkit = null) {
    ...
    form.onsubmit = function(e) {
        ...
        if (type === 'Multiple Forms + Sheets') {
            // Check if at least one form URL and one sheet URL is provided
            let hasFormUrl = false;
            let hasSheetUrl = false;
            for (let i = 1; i <= 6; i++) {
                if (formData.get(`form_url_${i}`)) hasFormUrl = true;
                if (formData.get(`sheet_url_${i}`)) hasSheetUrl = true;
            }
            if (!hasFormUrl || !hasSheetUrl) valid = false;
        } else if (type === 'Video' || type === 'Slides' || type === 'Folder') {
    ...
    if (toolkit) {
        ...
        // Handle Multiple Forms + Sheets type
        if (toolkit.type === 'Multiple Forms + Sheets') {
            // Parse JSON from form_url and response_url
            try {
                const formUrls = JSON.parse(toolkit.formUrl || '[]');
                const sheetUrls = JSON.parse(toolkit.responseUrl || '[]');
                
                // Populate form URLs
                for (let i = 0; i < formUrls.length && i < 6; i++) {
                    const formInput = document.getElementById(`form_url_${i + 1}`);
                    if (formInput) formInput.value = formUrls[i];
                }
                
                // Populate sheet URLs
                for (let i = 0; i < sheetUrls.length && i < 6; i++) {
                    const sheetInput = document.getElementById(`sheet_url_${i + 1}`);
                    if (sheetInput) sheetInput.value = sheetUrls[i];
                }
            } catch (e) {
                console.error('Error parsing JSON:', e);
            }
...
function updateToolkitTypeFields() {
    ...
    const multipleGroup = document.getElementById('multiple_forms_sheets_group');
